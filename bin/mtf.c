...
void lies_dumpfile(void) {
    ...
    if (k[0] != nn) {
        fprintf(stderr, "Logischer Fehler in lies_dumpfile!\n");
        schluss();
    }
    if ((gelesen = fread(k, sizeof (KNOTEN), 1L, recoverfile)) < 1L) {
        lesefehler(recoverfilename, 1L, gelesen, False);
    ...
    for (i = 3; i <= nn; i++) {
        if ((gelesen = fread(k, sizeof (KNOTEN), 8L, recoverfile)) < 8L) {
            lesefehler(recoverfilename, 8L, gelesen, False);
        } /* Zweige */
    ...
    if ((gelesen = fread(k, sizeof (KNOTEN), 4L, recoverfile)) < 4L) {
        lesefehler(recoverfilename, 4L, gelesen, False);
    } /* Zeit */
...
void weiterbetten(GRAPH testg, ADJAZENZ testadj, GRAPH g, ADJAZENZ adj, ADJAZENZ bild,
...

 {
    ...
    if (testadj[bestvertex] == 0) {
        *eingebettet = 1;
        return;
    } /* der Rest sind nur noch isolierte Knoten */
...
/* nautyg ist gleich g, aber nochmal in nauty-repraesentierung */ {
    ...
    for (i = g[0][0]; (i > 1) && getauscht; i--) {
        getauscht = 0;
        for (j = 1; j < i; j++) if (adj[grhf[j + 1]] > adj[grhf[j]]) {
                getauscht = 1;
                puffer = grhf[j];
                grhf[j] = grhf[j + 1];
                grhf[j + 1] = puffer;
...
/* Bubblesort */
 {
    ...
    for (i = 0; (i < wieviel) && getauscht; i++) {
        getauscht = 0;
        for (j = anzahl - 1; j > i; j--) if (tr[j - 1] < tr[j]) {
                getauscht = 1;
                puffer = tr[j];
                tr[j] = tr[j - 1];
                tr[j - 1] = puffer;
...
int trysubgraph(graph menge, GRAPH_STRUCT *lg, GRAPH_STRUCT *sg)
 {
    ...
    i = -1;
    for (j = 1; (i = nextelement(&menge, 1, i)) >= 0; j++) {
        bild[i] = j;
        urbild[j] = i;
    }
    knotenzahl = j - 1;
    ...
    for (j = 1; j < knotenzahl; j++) /* < reicht, da fuer == eh alle Nachbarn kleiner sind */ {
        ...
        i = -1;
        while ((i = nextelement(nautyg + vertex, 1, i)) >= 0)
        ...
        for (; (anzahl_moeglich >= rest_mengengroesse) && !(*erfolg); naechster++) {
            ...
            if (neue_kanten <= rest_kantenzahl) {
                ...
                if (neue_kanten == rest_kantenzahl) /* dann muss moegliche_knoten neu belegt werden */ {
                    run = -1;
                    while ((run = nextelement(&menge, 1, run)) >= 0) moegliche_knoten &= ~(largeg->nautyg[run]);
...
/* Vorgehensweise: Mengen -- fast -- in lexikographischer Ordnung erzeugen, sobald die
   ...
   das Sortieren wuerde sich nicht auszahlen. */
 {
    ...
    for (i = knotenzahl; (i > 1) && getauscht; i--) {
        getauscht = 0;
        for (j = 1; j < i; j++) if (adj[rhf[j + 1]] < adj[rhf[j]]) {
                getauscht = 1;
                puffer = rhf[j];
                rhf[j] = rhf[j + 1];
                rhf[j + 1] = puffer;
    ...
    if (kantenzahl > 0) {
        for (i = 1; (anzahl_moeglich >= mengengroesse) && !erfolg; i++)
            ...
            DELELEM1(&moegliche_knoten, puffer);
            anzahl_moeglich--;
...
/* geht davon aus, dass g->g und g->adj schon belegt sind und belegt 
   den Rest */
 {
    ...
    if (2 * summe > testzahl) g->more_than_half = 1;
    else g->more_than_half = 0;

    testzahl *= DENSE_SATZ;
    testzahl /= 100;
    if (summe >= testzahl) g->dense = 1;
    else g->dense = 0;
    ...
    for (i = 1; i <= knotenzahl; i++) {
        ...
        for (j = 1, run = 0; j <= knotenzahl; j++) {
            if ((run < (g->adj[i])) && ((g->g)[i][run] == j)) {
                ADDELEM1((g->nautyg) + i, j);
                run++;
            } else {
                ADDELEM1((g->inv_nautyg) + i, j);
...
void nauty_to_graph(graph *nautyg, GRAPH gr, ADJAZENZ adj, graph knotenzahl) {
    ...
    if ((knotenzahl % (8 * sizeof (graph))) == 0) m = knotenzahl / (8 * sizeof (graph));
    else m = knotenzahl / (8 * sizeof (graph)) + 1;


    for (i = 0; i <= knotenzahl; i++) { /*for (j=0; j<MAXN2; j++) gr[i][j]=leer;*/
        adj[i] = 0;
    }
    ...
    if (rest_kantenzahl > 0) /* noch keine verbotenen knoten */ {
        for (; (anzahl_moeglich >= rest_mengengroesse) && !(*erfolg); naechster++) {
            ...
            if (neue_kanten <= rest_kantenzahl) {
                ...
                if (neue_kanten == rest_kantenzahl) /* dann muss moegliche_knoten neu belegt werden */ {
                    run = -1;
                    while ((run = nextelement(&mm, 1, run)) >= 0) moegliche_knoten &= ~(largenaut[run]);
                    anzahl_moeglich = POPCOUNT(moegliche_knoten);
                }
                /* alle Nachbarn von Knoten in der Menge sind nicht mehr moeglich */
                if (rest_mengengroesse - 1) {
...
void gib_festspeicher_frei(MEMORY *first, size_t used_in_first) {
    ...
    if (first == nil) { /* alles loeschen */
        while (firstfestmem) {
            ...
            if (firstfestmem->memory) {
                free(firstfestmem->memory);
            }
            free(firstfestmem);
            firstfestmem = mem;
        }
...
void speichere_graphen_aus_outputliste_in_files(void) {
    ...
    if (save) {
        schreibe_dumpfile(False);
    } /* weil die Graphen beim
    ...
    while (i < len) { /* noch nicht am Ende des Codes */
        if (i == 0) {
            ...
            if (t->more == nil) {
                ...
                for (j = (int) nn - (int) menge[i - 1] - 2; j >= 0; j--) {
                    (t->more)[j] = nil;
                }
            }
            tt = t->more;
...
void sortiere_menge2(KNOTEN *menge, KNOTEN len, KNOTEN *zielmenge) {
    ...
    while (len > 1 && tausch) {
        tausch = False;
        for (i = 0; i < len - 1; i++) {
            if (zielmenge[i] > zielmenge[i + 1]) {
                h = zielmenge[i];
                zielmenge[i] = zielmenge[i + 1];
                zielmenge[i + 1] = h;
                tausch = True;
            }
        } /* danach ist das groesste Element ganz hinten */
...
int suche_original(KANTE *liste, int listlen, INDEXELEM *firstindexmenge,
    ...
    while (firstindexmenge && erg == -1) {
        ...
        for (j = 0; j < listlen; j++) {
            if (!ISELEM1(&g[l2[k[j]].k1], l2[k[j]].k2)) {
                j = listlen + 1;
            }
        }
        if (j == listlen) {
            erg = i;
        } /* alle Kanten stimmten ueberein */
        i++;
...
BOOL vortest_m(graph *g, int n, KANTE *liste, int listlen, KANTE *l2,
    ...
    for (i = 0; i < len; i++) {
        ADDELEM1(&menge_mask, menge[i]);
    }
    for (i = 0; i < n - len; i++) {
        ADDELEM1(&komp_mask, komp[i]);
    }
    ...
    while (e) {
        if (i != *orig) { /* Original nicht mit sich selbst vergleichen */
            ...
            if (erg > 0) { /* e aus der Liste nehmen */
                ...
                if (i < *orig) {
                    (*orig)--;
                } /* "orig" verschiebt sich */
                if (e2) {
                    e2->next = e->next;
                } else {
                    *firstindexmenge = e->next;
                }
            } else if (erg < 0) {
                return (False);
            }
...
BOOL mache_nautytest(graph *g, int n, KANTE *l2, int anz, INDEXELEM
    ...
    for (i = 0; i < anz; i++) {
        ADDELEMENT(&g2[m2 * l2[i].k1], n + i);
        ADDELEMENT(&g2[m2 * (n + i)], l2[i].k1);
        ADDELEMENT(&g2[m2 * l2[i].k2], n + i);
        ADDELEMENT(&g2[m2 * (n + i)], l2[i].k2);
    }
    ...
    while (firstindexmenge) {
        ...
        for (j = 0; j < firstindexmenge->len; j++) {
            ADDELEMENT(&g2[m2 * (n + k[j])], i);
            ADDELEMENT(&g2[m2 * i], n + k[j]);
        }
        firstindexmenge = firstindexmenge->next;
        i++;
    }
    ...
    if (autom) {
        finde_orbitzahlen(n2, labrev, orbits, orbits);
    }
...
void gute_menge(graph *g, KNOTEN n, KNOTEN *menge, KNOTEN len, KNOTEN *val) {
    ...
    for (x = 0; x < len - 1; x++) {
        for (y = x + 1; y < len; y++) {
            if (ISELEM1(&g[menge[x]], menge[y])) {
                liste[listlen].k1 = menge[x];
                liste[listlen].k2 = menge[y];
                listlen++;
                DELELEM1(&g[menge[x]], menge[y]);
                DELELEM1(&g[menge[y]], menge[x]);
                val[menge[x]]--;
                val[menge[y]]--;
                if (val[menge[x]] < len - 1 || val[menge[y]] < len - 1) {
                    erg = False;
                    x = y = len;
                }
        ...
        for (y = 0; y < n; y++) {
            if (val[y] == len) { /* y hat gleiche Valenz wie der neue Knoten */
                ...
                if (summe < summe2) {
                    ...
                    if (!sortiert) {
                        sortiere_menge(valenzvektor, (int) len);
                        sortiert = True;
                    }
    ...
    if (erg == True && nautykandidaten) {
        ...
        g[n] = 0;
        for (x = 0; x < len; x++) {
            ADDELEM1(&g[menge[x]], n);
            ADDELEM1(&g[n], menge[x]);
        }
        ...
        while (i < nautykandidaten) { /* interessante Knoten durchgehen */
            ...
            for (x = 0; x <= n; x++) /* Nachbarmenge des Knotens ermitteln */ {
                if (kanon[i] != x && ISELEM1(&g[kanon[i]], x)) {
                    menge2[len2++] = x;
                }
            }
    ...
    if (erg == True) {
        speichere_menge(menge, len, kanon, nautykandidaten);
    }
}
...
void initialisiere(void) {
    ...
    for (i = 0; i <= knoten; i++) {
        ALL_MASK[i] = 0;
        for (k = 1; k <= i; k++) {
            ADDELEM1(&ALL_MASK[i], k);
        }
    }
    for (i = 0; i <= MAXN2; i++) {
        ALL_MASK2[i] = 0;
        for (k = 0; k < i; k++) {
            ADDELEM1(&ALL_MASK2[i], k);
        }
    }

    /* "zweig" und "recoverzweig" initialisieren: */
    for (i = 0; i <= MAXN2 + 1; i++) {
        zweig[i] = recoverzweig[i] = 0;
    }
    ...
    for (i = 0; i <= MAXN2; i++) {
        output_graphenzahl[i] = graphenzahl[i] = 0L;
    }
    ...
    if (ramseygraph_ausgabe_all) { /* gleichmaessige Aufteilung */
        for (j = 0; j < anz_testgraphen; j++) {
            ...
            if (j == anz_testgraphen - 1) /* gesamten restlichen Buffer zuweisen */ {
                ...
                outputbufferlen[j] = (size_t) (outputbuffergroesse / anz_testgraphen)
                        - (size_t) OUTPUTCODESIZE;
            }
        ...
        for (i = outputlevel; i <= nn + (ramsey); i++) {
            ...
            if (i == nn + (ramsey)) /* gesamten restlichen Buffer zuweisen */ {
                ...
                outputbufferlen[i] = MAX((size_t) OUTPUTCODESIZE * 3L,
...
void l_concat(ELEM *elem1, ELEM *elem2, LISTE **firstlist, LISTE **lastlist) {
    ...
    if (basis1 != basis2) { /* gleiche Liste => nichts passiert */
        ...
        if (basis2->prevlist) {
            basis2->prevlist->nextlist = basis2->nextlist;
        } else {
            *firstlist = basis2->nextlist;
        }
        if (basis2->nextlist) {
            basis2->nextlist->prevlist = basis2->prevlist;
        } else {
            if (lastlist) {
                *lastlist = basis2->prevlist;
            }
        }
    }
}
...
BOOL wende_testgraphen_an(graph *g, KNOTEN n, BOOL von_oben) {
    ...
    if (von_oben) {
                        ...
                        if (tg->ramsey < n) {
                            ...
                            if (n == nn) {
                                ...
                                if (ramseygraph_ausgabe) {
                                    gib_graphen_aus(g, n, n + 1);
                                    ramseyfilefilled = True;
                                }
                            }
                        }
                    }
        ...
        einandergeraten wuerde. */
                ...
                if (!(tg->maximalzahl_erreicht) || ramseygraph_ausgabe_all) {
                    l[--anf2] = *k;
                } /* Graph tg in die Liste aufnehmen */
                j = tg->obergraphen;
                while (*j) {
                    geprueft[*j] = True;
                    j++;
...
void sortiere_knoten(KNOTEN n, KNOTEN *val, KNOTEN *f) {
    ...
    for (i = 0; i < n; i++) { /* Knoten i einsortieren */
        ...
        while (liste[pos] && val[i] > val[liste[pos] - 1]) {
            pos = liste[pos];
        }
        liste[i + 1] = liste[pos];
        liste[pos] = i + 1;
...
void konstruiere(KNOTEN n, BOOL callnauty) {
    ...
    if (anzval < minval + 1) {
        ...
        if (erreicht == full[n - 1]) {
            ...
            spannende_menge(n, g, verboten, erreicht,
        ...
        if (erreicht == full[n - 1]) {
            gute_menge(g, n, menge, minval + 1, val);
        }
    }
    ...
    if (callnauty) {
        ...
        if (grpsizes && !recover) {
            options.userlevelproc = hole_groupsize;
            grpsize = 1L;
        } else {
            options.userlevelproc = NILFUNCTION;
        }
    ...
    while (l) {
        ...
        if (gute_menge_teil2(g, n, e->menge, e->len, kante, kantenlistlen)) {
            ...
            if (accept && local_density && n + 1 >= l_n) {
                ...
                if (!(lokale_dichte_gb(g, n + 1, sortiert))) {
                    accept = False;
                }
            }
        ...
        while (kantenlistlen > 0) { /* geloeschte Kanten wieder einfuegen */
            ...
            val[kante[kantenlistlen].k1]++;
            val[kante[kantenlistlen].k2]++;
        }
...
char read_old_or_new(FILE *f, unsigned char *lastinput, unsigned char s,
    ...
    if (*z >= s) { /* new number to be read from the file */
        if (fread(num, sizeof (unsigned char), 1, f) == 0) {
            return (2);
        }
...
char read_multi_code2_s_old(FILE *f, graph *g, KNOTEN *n) {
    ...
    if (fread(&s, sizeof (unsigned char), 1, f) == 0) {
        return (feof(f) ? 0 : 2);
    }
    if (s > maxentries) {
        return (3);
    ...
    z = 0;
    if (read_old_or_new(f, lastinput, s, &z, (unsigned char *) n) == 2) {
        return (2);
    }
    if (*n >= nn) {
        return (4);
    ...
    i = 0;
    while (i < (*n) - 1) {
        if (read_old_or_new(f, lastinput, s, &z, &num) == 2) {
            return (2);
        }
        if (num != 0) {
            ADDELEM1(&g[i], num - 1);
...
void erstelle_testgraph_kantenarray(void) {
    ...
    for (i = 0; i < LISTENGROESSE; i++) {
        testgraphkanten[i].end = 0;
        testgraphkanten[i].anf = anz_testgraphen + 1;
    }
    t = firsttestgraph;
    i = 1;
    while (t) {
    ...
    while (n > 0) { /* Graphen durchgehen */
        ...
        for (i = testgraphkanten[testgraphen[n]->kanten].end + 1;
            ...
            while (!untergraph && (testgraphen[i]->obergraphen)[j]) {
                if ((testgraphen[i]->obergraphen)[j] == n) {
                    untergraph = True;
                }
                j++;
            }
...
int main(int argc, char *argv[]) {
    ...
    for (i = 1; i < argc; i++) {
        switch (argv[i][0]) {
            case 'n':
            {
                if (strcmp(argv[i], (char *) "n") == 0) {
                    if (i + 1 < argc) {
                        ...
                        fprintf(stderr, "Option 'n': missing parameter!\n");
                        exit(0);
                    }
            ...
            case 'b':
            {
                if (strcmp(argv[i], (char *) "brandt") == 0) {
                    brandt = True;
                } else {
                    fprintf(stderr, "Unknown option %s!\n", argv[i]);
                    exit(0);
            ...
            case 'e':
            {
                if (strcmp(argv[i], (char *) "extinfo") == 0) {
                    extinfo = True;
                } else {
                    fprintf(stderr, "Unknown option %s!\n", argv[i]);
                    exit(0);
            ...
            case 'g':
            {
                if (strcmp(argv[i], (char *) "grpsize") == 0) {
                    grpsizes = True;
            ...
            case 'm':
            {
                if (strcmp(argv[i], (char *) "mod") == 0) {
                    if (i + 1 < argc) {
                        mod = atoi(argv[++i]);
                    } else {
                        fprintf(stderr, "Option 'mod': missing parameter!\n");
                        exit(0);
                    }
                } else {
                    fprintf(stderr, "Unknown option %s!\n", argv[i]);
            ...
            case 'c':
            {
                if (strcmp(argv[i], (char *) "class") == 0) {
                    if (i + 1 < argc) {
                        rest = atoi(argv[++i]);
                    } else {
                        fprintf(stderr, "Option 'rest': missing parameter!\n");
                        exit(0);
                    }
                } else {
                    fprintf(stderr, "Unknown option %s!\n", argv[i]);
            ...
            case 'l':
            {
                if (strcmp(argv[i], (char *) "level") == 0) {
                    if (i + 1 < argc) {
                        level = atoi(argv[++i]);
                    } else {
                        fprintf(stderr, "Option 'level': missing parameter!\n");
                        exit(0);
                    }
                } else if (strcmp(argv[i], (char *) "local_density") == 0) {
                    if (i + 2 < argc) {
                        local_density = True;
                        l_n = (KNOTEN) atoi(argv[++i]);
                        l_k = atoi(argv[++i]);
                    } else {
                        fprintf(stderr, "Option 'local_density': missing "
                                "parameter!\n");
                        exit(0);
                    }
                } else {
            ...
            case 'o':
            {
                if (strcmp(argv[i], (char *) "outputlevel") == 0) {
                    if (i + 1 < argc) {
                        outputlevel = atoi(argv[++i]);
                    } else {
                        fprintf(stderr, "Option 'outputlevel': missing "
                                "parameter!\n");
                        exit(0);
                    }
                } else {
            ...
            case 'f':
            {
                if (strcmp(argv[i], (char *) "file") == 0) {
                    if (i + 1 < argc) {
                        inputfilename = argv[++i];
                    } else {
                        fprintf(stderr, "Option 'file': missing parameter!\n");
                        exit(0);
                    }
                } else {
                    fprintf(stderr, "Unknown option %s!\n", argv[i]);
            ...
            case 'r':
            {
                if (strcmp(argv[i], (char *) "ramsey_kn") == 0) {
                    if (i + 1 < argc) {
                        ramsey_kn = atoi(argv[++i]);
                    } else {
                        fprintf(stderr, "Option 'ramsey_kn': missing "
                                "parameter!\n");
                        exit(0);
                    }
                } else if (strcmp(argv[i], (char *) "ramsey_kn-e") == 0) {
                    if (i + 1 < argc) {
                        ramsey_kn_e = atoi(argv[++i]);
                    } else {
                        fprintf(stderr, "Option 'ramsey_kn-e': missing "
                                "parameter!\n");
                        exit(0);
                    }
                } else if (strcmp(argv[i], (char *) "ramsey") == 0) {
                    if (i + 1 < argc) {
                        ramseyfilename = argv[++i];
                    } else {
                        fprintf(stderr, "Option 'ramsey': missing filename!\n");
                        exit(0);
                    }
                    if (i + 1 < argc && argv[i + 1][0] >= '0' && argv[i + 1][0] <= '9') {
                        ramsey_min = atol(argv[++i]);
                        r_min = True;
                        if (i + 1 < argc && argv[i + 1][0] >= '0' && argv[i + 1][0] <= '9') {
                            ramsey_max = atol(argv[++i]);
                            r_max = True;
                        }
                    }
                } else if (strcmp(argv[i], (char *) "reverse") == 0) {
                    revert = False;
                } else {
                    fprintf(stderr, "Unknown option %s!\n", argv[i]);
                    exit(0);
            ...
            case 's':
            {
                if (strcmp(argv[i], (char *) "stdout") == 0) {
                    standardout = True;
                } else {
                    fprintf(stderr, "Unknown option %s!\n", argv[i]);
                    exit(0);
            ...
            case 'w':
            {
                if (strcmp(argv[i], (char *) "write_ramseygraph") == 0) {
    ...
    if (k < 4) {
        fprintf(stderr, "Impossible or missing vertex number"
                " (minimum 4). \n");
        exit(0);
    }
    ...
    if (ramseyfilename) { /* Option "ramsey" wird gewuenscht */
        if (ramsey_kn) {
            fprintf(stderr, "Error: Options 'ramsey' and 'ramsey_kn' "
                    "used simultaneously!\n");
            exit(0);
        }
        if (ramsey_kn_e) {
            fprintf(stderr, "Error: Options 'ramsey' and 'ramsey_kn-e'"
                    " used simultaneously!\n");
            exit(0);
        }
        if (strcmp(ramseyfilename, (char *) "stdin") == 0) {
            ramseyfile = stdin;
        ...
        if (ramsey_kn && ramsey_kn_e) {
            fprintf(stderr, "Error: Options 'ramsey_kn'"
    ...
    if (ramseylevel_start > nn) {
        ramseylevel_start = nn;
    }


    /* Optionen "local_density", "outputlevel", "mod", "rest" und "level" 
       ueberpruefen: */
    if (outputlevel < 4 || ramseygraph_ausgabe) {
        outputlevel = nn + (ramsey);
    }
    /* bei Ramseyzahlen ist auch die naechsthoehere Knotenzahl interessant */

    if (level != -1) {
        if (mod == -1) {
            fprintf(stderr, "Missing mod-parameter!\n");
            exit(0);
        }
        if (rest == -1) {
            fprintf(stderr, "Missing class-parameter!\n");
            exit(0);
        }
        if (level < 4) {
            fprintf(stderr, "Level parameter (%d) out of range!\n", level);
            exit(0);
        } /* falls level==3, so muesste schon innerhalb der
                               Funktion "main" entschieden werden, ob 
                               "konstruiere" aufgerufen werden darf */
    }
    if (rest != -1) {
        if (rest < 0) {
            fprintf(stderr, "Invalid class-parameter!\n");
            exit(0);
        }
        if (mod == -1) {
            fprintf(stderr, "Missing mod-parameter!\n");
    ...
    if (mod != -1) {
        if (mod < 1) {
            fprintf(stderr, "Invalid mod-parameter!\n");
            exit(0);
        }
        if (rest == -1) {
            fprintf(stderr, "Missing class-parameter!\n");
        ...
        if (level == -1) {
            fprintf(stderr, "Missing level-parameter: "
                    "I will try level %d.\n", (level = k * 2 / 3));
        }
    }
    ...
    if (inputfilename) {
        strcat(filenamenoptionen, (char *) "_IFILE_");
        strcat(filenamenoptionen, inputfilename);
    }
    if (twin) {
        strcat(filenamenoptionen, (char *) "_TWIN");
    }
    if (groetzsch) {
        strcat(filenamenoptionen, (char *) "_GR");
    }
    if (ramsey) {
        strcat(filenamenoptionen, (char *) "_RAMSEY_");
        strcat(filenamenoptionen, ramseyfilename);
        if (r_min) {
            sprintf(buildstring, "_F%u", ramsey_min);
            strcat(filenamenoptionen, buildstring);
        }
        if (r_max) {
            sprintf(buildstring, "_L%u", ramsey_max);
            strcat(filenamenoptionen, buildstring);
        }
        if (ramseygraph_ausgabe) {
            strcat(filenamenoptionen, (char *) "_WRG");
        }
        if (ramseygraph_ausgabe_all) {
            if (ramseylevel_start == nn) {
                sprintf(buildstring, (char *) "_WRGA");
            } else {
                sprintf(buildstring, (char *) "_WRGA%u", ramseylevel_start);
            }
            strcat(filenamenoptionen, buildstring);
        }
    }
    ...
    if (ramsey_kn) {
        sprintf(buildstring, "_RAMSEY_K%d", ramsey_kn);
        strcat(filenamenoptionen, buildstring);
    } else if (ramsey_kn_e) {
        sprintf(buildstring, "_RAMSEY_K%d-e", ramsey_kn_e);
        strcat(filenamenoptionen, buildstring);
    } else if (local_density) {
        sprintf(buildstring, "_LOCDEN_%d_%d", l_n, l_k);
        strcat(filenamenoptionen, buildstring);
    }
    if (mod != -1) {
        sprintf(buildstring, "_MOD%d.%d_L%d", rest, mod, level);
        strcat(filenamenoptionen, buildstring);
    }
    if (revert == False) {
        strcat(filenamenoptionen, (char *) "_REV");
    }
    if (grpsizes) {
        strcat(filenamenoptionen, (char *) "_GS");
    }
    ...
    if (standardout) {
        strcat(recoverfilename, "_STDOUT");
    }
    ...
    if (ramsey) {
        ...
        while (inputnumber <= ramsey_max &&
            ...
            switch (inputerg) {
                ...
                case 2:
                {
                    fprintf(stderr, "Error while reading graph %ld!\n", inputnumber);
                    schluss();
                }
                case 3:
                {
                    fprintf(stderr, "Input graph %ld out of range!\n", inputnumber);
                    schluss();
                }
                case 4:
                {
                    fprintf(stderr, "Input graph %ld too big!\n", inputnumber);
                    schluss();
                }
            }
        }
    ...
    if (ramseygraph_ausgabe_all) { /* Bei Ausgabe aller Ramseygraphen:
        ...
        for (i = 0; i < (int) anz_testgraphen; i++) { /* Testgraphen durchlaufen */
            ...
            if (standardout) {
                sprintf(outputfilename[i], "stdout");
            }
        ...
        for (i = outputlevel; i <= nn + (ramsey); i++) {
            if (i == nn + 1) {
                sprintf(outputfilename[i], "MTF_N+%d", i);
            } else {
                sprintf(outputfilename[i], "MTF_N%d", i);
            }
            strcat(outputfilename[i], filenamenoptionen);
    ...
    if (inputfilename) {
        if (strcmp(inputfilename, (char *) "stdin") == 0) {
            inputfile = stdin;
    ...
    if (save) {
        schreibe_dumpfile(True);
    } /* das ist wichtig, damit man von dem
